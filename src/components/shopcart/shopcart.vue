  ...
  <div>
    <div class="shopcart">
      ...
      <div class="ball-container">
        <!--每个小球的动画是独立的，故都有自己的transition标签-->
...
<script>
  ...
  export default{
    ...
    computed: {
      ...
      //购物车的显示和隐藏由isShow和totalCount共同决定
      listShow(){
        if(this.totalCount === 0){
          this.isShow = false//修改bug
          return false
        }
  
        //要显示列表的时候生成scroll对象
        
        // 如何创建一个单例(单一的实例)对象(不然scroll对象会被多次创建，产生bug)
    ...
    methods:{
      ...
      pay(){
        let {totalPrice,minPrice,deliveryPrice} = this
        if(totalPrice >=minPrice){
          alert('支付' + (totalPrice + deliveryPrice))
        }
      },
      toggleShow(){
        //如果当前数量为0, 不切换（虽然不影响效果，但数据与页面须一致）
        if(this.totalCount===0) {
          return
        }
