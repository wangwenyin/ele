  ...
  export default{
    ...
    methods: {
      toggleFavorite(){
        ...
        //保存到localStorage(防止刷新后没有了)
        localStorage.setItem('favorite',this.favorite)// 'true'/'false'会以字符串(以前是json字符串)的形式
      },
      ...
      _initScroll(){
        this.$nextTick(()=>{
          ...
          //给ul设置样式宽度(内容的宽度要大于容器的宽度，滚动才生效)
          let ul = this.$refs.ul
          let liWidth = 120
          let space = 6
          let liCount = this.seller.pics.length
    ...
    created(){
      // 如果seller还没有数据不能显示seller
      if(!this.seller.pics) {
        return
      }
  
      this._initScroll() // 从其它路由切换来过
    },
    watch: { // 监视seller的变化
      seller (newSeller, oldSeller) { // 在当前路由刷新
        this._initScroll()
      }
    }
  }
